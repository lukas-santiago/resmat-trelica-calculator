...
export type VectorComponentsGroups = {
    grid: Group
    ruler: Group
    bars: Group
    supports: Group
...
export class VectorComponent {
    constructor(private groupName: keyof VectorComponentsGroups) { }

    public get two(): Two {
        return VectorProcessor.two;
    }
    public get group(): Group {
        return VectorProcessor.groups[this.groupName];
    }

}

export class Point extends VectorComponent {
    static all: any = []
    ...
    id: any;
    text: any;
    get x() { return (this.two.width / Grid.columns) * (this.column - 1) }
    get y() { return (this.two.height / Grid.rows) * (this.row - 1) }
    ...
    drawSelf() {
        this.circle = this.two.makeCircle(this.x, this.y, 8)
    ...
    usePoint(color: any, text: any, used: any) {
        if (this.used !== true) {
    ...
    discardPointFromBar(bar: any) {
        if (this.objectsIn.bars.length == 1) {
            this.objectsIn.forces.forEach((f: { destroy: () => any; }) => f.destroy())
            this.objectsIn.bars = []
    ...
    drawText() {
        ...
        let tag = 'a'
        let usedQuantity = Point.all.filter((p: { used: boolean; }) => p.used === true).length
        for (let i = 0; i < usedQuantity; i++) {
            tag = ((parseInt(tag, 36) + 1).toString(36)).replace(/0/g, 'a')
        }
        this.letra = tag.toLocaleUpperCase()
        this.text = this.two.makeText(this.letra, this.x - 15, this.y - 12)
        this.text.classList.push('svg-text')

        VectorProcessor.groups.text.add(this.text)
    }

    static allToString() {
        let string = ''
        for (let row = 1; row <= Grid.rows + 1; row++) {
            let inThisRow = Point.all.filter((p: { row: number; }) => p.row == row)
            inThisRow.forEach((p: { used: boolean; letra: any; }) =>
    ...
    static possiblePointsInPath(from: any, to: { column: any; row: any; }, returnArray: any = []) {
    ...
    static pickOne(from: any, to: any) {
        let p: any = {}
    ...
    static get(row: any, column: any) { return Point.all.find((p: { row: any; column: any; }) => p.row == row && p.column == column) }
}

export class Grid extends VectorComponent {
    static columns: number
    static rows: number
    static rulerGroup: any
    static gridLinesGroup: Group
    ...
    drawSelf() {
        Grid.gridLinesGroup = this.two.makeGroup()
        Grid.gridLinesGroup._className = 'grid-group'
        this.group.add(Grid.gridLinesGroup)

        this.drawGrid()
    }

    drawGrid() {
        for (let x = 0; x <= this.two.width + 1; x += this.two.width / Grid.columns) {
            let lineX = this.two.makeLine(x, 0, x, this.two.height)
            lineX.linewidth = 1
            lineX.stroke = '#6c757d'
            Grid.gridLinesGroup.add(lineX)
        }
        for (let y = 0; y <= this.two.height + 1; y += this.two.height / Grid.rows) {
            let lineY = this.two.makeLine(0, y, this.two.width, y)
            lineY.linewidth = 1
            lineY.stroke = '#6c757d'
...
export class Ruler extends VectorComponent {
    static topRulerGroup: Group
    static sideRulerGroup: Group
    static widthDistance: number
    static heightDistance: number

    constructor(widthDistance: any, heightDistance: any) {
        super('ruler');
    ...
    drawSideRuler() {
        Ruler.sideRulerGroup = this.two.makeGroup()
        this.group.add(Ruler.sideRulerGroup)

        let mainLine = this.two.makeLine(this.two.width + 30, 0, this.two.width + 30, this.two.height)
        mainLine.stroke = '#545b62'
        Ruler.sideRulerGroup.add(mainLine)

        for (let y = 0, row = Grid.rows; y <= this.two.height + 1; y += this.two.height / Grid.rows, row--) {
            let auxLine = this.two.makeLine(this.two.width + 25, y, this.two.width + 35, y)
            ...
            let textX = this.two.makeText(textValue == 'Infinity' ? '0' : textValue, this.two.width + 47, y)
    ...
    drawTopRuler() {
        Ruler.topRulerGroup = this.two.makeGroup()
        this.group.add(Ruler.topRulerGroup)

        let mainLine = this.two.makeLine(0, -30, this.two.width, -30)
        mainLine.stroke = '#545b62'
        Ruler.topRulerGroup.add(mainLine)

        for (let x = 0, col = 0; x <= this.two.width + 1; x += this.two.width / Grid.columns, col++) {
            let auxLine = this.two.makeLine(x, -25, x, -35)
            auxLine.stroke = '#545b62'
            Ruler.topRulerGroup.add(auxLine)

            let textValue = ((Ruler.widthDistance / Grid.columns) * col).toFixed(2)//.replace(/([.,]00)|([.,]\d0)$/, "")
            let textX = this.two.makeText(textValue == 'Infinity' ? '0' : textValue, x, -45)
            textX.classList.push('ruler-text')
